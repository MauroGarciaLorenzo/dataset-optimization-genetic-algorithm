...
# --- 1. PG DEFINITIONS AND OPERATORS ---

# Set of Functions and Terminals (For feature synthesis)
FUNCTIONS = ['+', '-', '*', '/_prot', 'sen', 'cos']
...
def prot_div(x1, x2):
    ...
    # Returns 1 if the divisor is near zero, otherwise x1/x2.
...
# --- 2. GENETIC PROGRAMMING (GP) AUXILIARY FUNCTIONS ---
...
def crossover_tree(parent1_tree: Node, parent2_tree: Node):
    ...
    # 1. Deep copy of tree 1 (will be the child)
    child = copy.deepcopy(parent1_tree)

    # 2. Find random nodes in the child and in parent 2
...
def mutate_tree(tree_node: Node):
    ...
    if node_to_mutate:
        # Generate a new random subtree (limited depth, e.g., 3)
...
# --- 3. GENETIC ALGORITHM (GA) CONFIGURATION AND FUNCTIONS ---
...
def crossover_hybrid(p1, p2, n_orig_attrs):
    ...
    new_num_syn_attr = int(floor((p1.num_syn_attr + p2.num_syn_attr) / 2))
...
def mutate_hybrid(individual, n_orig_attrs):
    ...
    if random() < 0.5:  # 50% probability of mutating the mask
    ...
    if individual.num_syn_attr > 0 and random() < 0.5:  # 50% probability of mutating a tree
...
def run_hybrid_evolutionary_regression(csv_path):
    ...
    for generation in range(NUM_GENERATIONS):
        ...
        # 4.4. REPLACEMENT (Generational Elitist Strategy)
    ...
    print(f"Total Features Used: {final_best.num_total_attr}")
